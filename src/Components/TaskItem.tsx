          ...
          <div key={index} className="mt-5 bg-slate-300 p-2 rounded cursor-pointer">
            <div className="flex justify-between relative">
              {isEditing === index ? (
                ...
                <h5 className={`mt-5 bg-slate-300 p-2 rounded md:text-3xl ${completedTasks[index] ? "line-through opacity-50" : ""}`}>
                  {task.name || "Learn"}
                </h5>
              )}
              <svg
            ...
            <div className="flex justify-between py-2">
              <p className={`mt-5 bg-slate-300 p-2 max-md:text-2xl rounded ${completedTasks[index] ? "line-through opacity-50" : ""}`}>
                {task.time || "12:00pm"}
              </p>
              <input
                type="checkbox"
